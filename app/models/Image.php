	...
	public function crop($filename,$max_width = 700,$max_height = 700)
	{
		...
		if(file_exists($filename))
		{
			...
			$src_y = 0;
 			if($max_width > $max_height)
			{
				$src_y = round(($src_h - $max_height) / 2); //crop at middle
				//$src_y = round(($src_h - $max_height)); crop at bottom
				//$src_y = 0; crop at top
			}else
			{
				$src_x = round(($src_w - $max_width) / 2); //crop at center
				//$src_x = round(($src_w - $max_width)); crop at the end
				//$src_x = 0; crop at the begining
			}
